...
1. **在结构中**手持**结构标记器**右键：界面会列出你当前位置命中的所有结构，选中一个即把维度、位置与边界写进标记，并算出战利品期望。散落的容器改用**宝箱标记器**：按 `V` 分析并标记准星指向的容器。
...
原版结构的战利品表来源已全部收录：34 个原版结构各有权威映射，其中 4 个确认不放置任何战利品容器（海底神殿 `ocean_monument`、沼泽小屋 `swamp_hut`、下界化石 `nether_fossil`、古迹废墟 `trail_ruins`），它们给出精确的 0 产出而不是「不支持」；试炼密室 `trial_chambers` 的宝库（含不祥宝库）与试炼刷怪笼产出、废弃传送门 `ruined_portal` 与沉船 `shipwreck` 的箱子也都在解析范围内。

采掘器产出按已分析的物品期望构造奖励权重。它保留长期期望值，但不复现原版 LootTable 的联合随机分布、奖池选择、函数链或随机序列。

破坏**结构采掘器**或**结构数据操作仪**时，里面的结构标记、写入槽与尚未走完输出通道的产出都会掉落出来，不会随方块一起消失（罐里的加工用流体不返还）。
...
**宝箱标记器**用于处理散落的容器：手持它按 `V`（默认按键）分析准星指向的容器，若该容器带 LootTable 引用，就直接以该战利品表为来源做期望分析并标记该容器。分析走按键而不是右键，因为右键一个宝箱会优先打开它；右键宝箱标记器只打开分析界面查看已标记内容（宝箱价值、计算方式）。

宝箱标记器与结构标记器共用同一套标记 NBT（`StructureMarkerData`，宝箱另在其下写一个 `ChestData` 子标签记录 LootTable 与 seed）；下游只认 NBT 结构、不认物品类型，因此数据操作仪的复制模式和采掘器的生产链路都能直接消费它。两者唯一的差异在分析：宝箱不注册结构，它的 profile 只用 NBT 里记录的 LootTable，不查结构模板、不依赖世界是否已加载。

界面上的期望数量优先来自战利品表的**精确期望计算**：整张表能被建模成有限概率空间时，显示的是精确值（状态「精确」）。少数当前无法精确建模的机制（附魔枚举关闭、未支持的战利品函数等）会退化为**采样估计**并标为「近似」，而不是伪装成精确值；只有连估计都给不出的情况才标「不支持」。**当前附魔产出不计入期望值**：附魔枚举处于关闭状态，`enchant_with_levels` 被当作不产出附魔的物品，因此含附魔的战利品表，其显示数量会略低于实际可能产出（原因与影响见 [docs/code-wiki.md](docs/code-wiki.md) §13）。
...
| `item/`               | 结构标记器/宝箱标记器、扳手、维度拆解核心、数据整合器、结构阐释器、碎片与采掘代币 |
...
改多方块几何必须重新核对计数、重叠、包围盒覆盖、D4 对称、面连通与机头契约；`StructureMinerMultiblock` 的四个集合与 `projectionCounts()` 是唯一事实来源，不要手数。原工程用 `docs/tools/multiblock_geometry_check.py` 自动校验这些不变量，该脚本**未随移植迁入**，因此目前只能手工核对（见 [docs/code-wiki.md](docs/code-wiki.md) §13）。
