 ...
 * "what is in here", the strip under it answers "how far along is it, and where does its
 * parallelism come from". The second would not fit anywhere on a 20px cell, so it is hover-only
 * rather than a third collapsible control.
 *
 * <p>Units are load-bearing. Total and efficiency parallelism are whole units; external
 * acceleration parallel is stored in hundredths and must go through {@link
 * StructureMinerScreen#formatDecimal}.
 */
final class StructureMinerTooltips {
    private StructureMinerTooltips() {}

    /** Slot contents, where the marker points, and what a click will do. */
    static void markerSlot(
            StructureMinerScreenContext c, GuiGraphics g, int slot, int screenX, int screenY) {
     ...
     * flips a thread's enabled flag; the info-page selector reuses the same grid but only selects,
     * so it keeps the plain enable/disable state without the misleading right-click instruction.
     */
    static void markerSlot(
        ...
        Component name =
                info.map(markerInfo -> TranslateHelper.structureName(markerInfo.structure().id()))
                        .orElse(
        ...
        if (info.isPresent() && slot >= 0 && slot < telemetry.markers().size()) {
            StructureMinerTelemetrySnapshot.Marker marker = telemetry.markers().get(slot);
        ...
        lines.add(
                Component.translatable(
                        rightClickToggle
                                ...
                                : (menu.isMarkerSlotEnabled(slot)
                                        ? "screen.dimension_tech.structure_miner.slot.disable"
                                        : "screen.dimension_tech.structure_miner.slot.enable")));
    ...
    static void markerProgress(
            StructureMinerScreenContext c, GuiGraphics g, int slot, int screenX, int screenY) {
        StructureMinerMenu menu = c.menu();
        List<StructureMinerTelemetrySnapshot.Marker> markers = menu.telemetrySnapshot().markers();
        if (slot < 0 || slot >= markers.size()) return;
        StructureMinerTelemetrySnapshot.Marker marker = markers.get(slot);
        ...
        if (marker.actualTicks() != marker.naturalTicks()) {
            lines.add(
                    Component.translatable(
                            "screen.dimension_tech.structure_miner.marker_info.actual_ticks",
                            marker.actualTicks()));
        }
        ...
        if (marker.processingTime() <= 0) {
            lines.add(
                    Component.translatable("screen.dimension_tech.structure_miner.marker.no_plan"));
        }
