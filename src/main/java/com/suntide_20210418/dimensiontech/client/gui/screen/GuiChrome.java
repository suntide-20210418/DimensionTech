...
final class GuiChrome {
    ...
    /** A value cell on a light face, with a one pixel accent rail down its left edge. */
    static void metricCard(GuiGraphics g, int x, int y, int width, int height, int accent) {
        fill(g, x, y, width, height, GuiPalette.HAIRLINE);
        fill(g, x + 1, y + 1, width - 2, height - 2, GuiPalette.FACE);
        fill(g, x + 1, y + 1, width - 2, 1, GuiPalette.HILIGHT);
        fill(g, x + 1, y + 1, 2, height - 2, accent);
        fill(g, x + 1, y + height - 2, width - 2, 1, GuiPalette.SHADE);
        fill(g, x + width - 2, y + 1, 1, height - 2, GuiPalette.SHADE);
    }
    ...
    static void tab(
        ...
        if (selected) {
            ...
            g.drawCenteredString(font, label, x + width / 2, y + 3, GuiPalette.INK);
            return;
        }
        fill(g, x, y, width, 13, GuiPalette.STRIPE_INK);
        fill(g, x, y, width, 1, GuiPalette.SHADE);
        g.drawCenteredString(font, label, x + width / 2, y + 3, GuiPalette.DIM);
    }
    ...
    static void statusChip(
        ...
        g.drawCenteredString(font, label, x + width / 2, y + 2, GuiPalette.TEXT);
    }
    ...
    static void listRow(
            ...
            int accent) {
        int face =
                selected ? GuiPalette.SELECT : hovered ? GuiPalette.STRIPE_WELL : GuiPalette.WELL;
        fill(g, x, y, width, 14, face);
        if (selected || hovered) fill(g, x, y, 2, 14, selected ? accent : GuiPalette.SHADE);
        g.drawString(
                font, label, x + 6, y + 3, selected ? GuiPalette.TEXT : GuiPalette.TEXT, false);
    }
    ...
    static void emptyState(
        ...
        g.drawCenteredString(
                font, label, x + width / 2, y + Math.max(0, height / 2 - 4), GuiPalette.DIM);
    }
    ...
    static void confirmationModal(
        ...
        g.drawCenteredString(font, title, x + width / 2, y + 5, GuiPalette.INK);
        g.drawCenteredString(font, message, x + width / 2, y + 25, GuiPalette.DIM);
