...
public final class StructureDataOperatorMenu extends AbstractContainerMenu {
    public static final int SOURCE_MARKER_MENU_SLOT =
        ...
        for (int row = 0; row < 3; row++)
            for (int column = 0; column < 9; column++)
                addSlot(
                        new OperatorSlot(
                                be.inventory(),
                                StructureDataOperatorBlockEntity.OPERAND_START + row * 9 + column,
                                8 + column * 18,
                                116 + row * 18,
                                () -> operationPage));
        addSlot(
            ...
            for (int col = 0; col < 9; col++)
                addSlot(
                        new PlayerSlot(
                                ...
                                () -> !operationPage));
        for (int col = 0; col < 9; col++)
            addSlot(new PlayerSlot(inv, col, 79 + col * 18, 316, () -> !operationPage));
        for (int row = 0; row < 3; row++)
            for (int col = 0; col < 9; col++)
                addSlot(
                        new PlayerSlot(
                                ...
                                () -> operationPage));
        for (int col = 0; col < 9; col++)
            addSlot(new PlayerSlot(inv, col, 8 + col * 18, 256, () -> operationPage));
    }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (index < PLAYER_SLOT_START) {
            if (!moveItemStackTo(stack, PLAYER_SLOT_START, slots.size(), true)) return empty;
        } else if (!moveItemStackTo(
                stack, 0, StructureDataOperatorBlockEntity.INVENTORY_SIZE, false)) return empty;
        if (stack.isEmpty()) slot.set(ItemStack.EMPTY);
        else slot.setByPlayer(stack);
        return stack;
    }
