 ...
 *
 * <p>Kept static and dependency-free so the rounding rules — which is where progress bars usually
 * go wrong — can be asserted by a plain unit test instead of by looking at the screen.
 */
final class StructureMinerProgressStrip {
     ...
     *
     * <p>{@code progress} is a logical counter that keeps climbing across cycles, so it is clamped
     * to a single cycle before scaling. Without that clamp a slot that has been running for several
