...
public final class StructureReactorFluidContainerGameTests {
    private static final BlockPos REACTOR = new BlockPos(1, 2, 1);
    private static final int BUCKET = 1_000;

    private StructureReactorFluidContainerGameTests() {}
    ...
    public static void aFilledContainerFeedsTheInputTank(GameTestHelper helper) {
        ...
        helper.assertTrue(
                reactor.exchangeWithFluidContainer(water), "the transfer must move fluid");
        helper.assertTrue(
                reactor.inputTank().getFluidAmount() == BUCKET,
                "the input tank must hold the bucket, held "
                        + reactor.inputTank().getFluidAmount());
        helper.assertTrue(
                reactor.inputTank().getFluid().getFluid() == Fluids.WATER,
    ...
    public static void anEmptyContainerDrawsTheOutputTank(GameTestHelper helper) {
        StructureReactorBlockEntity reactor = place(helper);
        reactor.outputTank().fill(new FluidStack(Fluids.WATER, BUCKET), FluidAction.EXECUTE);
        IFluidHandlerItem bucket = container(new ItemStack(Items.BUCKET));

        helper.assertTrue(
                reactor.exchangeWithFluidContainer(bucket), "the transfer must move fluid");
        helper.assertTrue(
                reactor.outputTank().isEmpty(),
                "the output tank must be emptied, left " + reactor.outputTank().getFluidAmount());
        helper.assertTrue(
    ...
    public static void aFilledContainerMatchesTheTankItMayUse(GameTestHelper helper) {
        ...
        IFluidHandlerItem water = container(new ItemStack(Items.WATER_BUCKET));
        helper.assertTrue(
                reactor.exchangeWithFluidContainer(water),
                "a container matching the input fluid must feed the input tank");
        helper.assertTrue(
                reactor.inputTank().getFluidAmount() == BUCKET * 2,
        ...
        helper.assertFalse(
                reactor.exchangeWithFluidContainer(lava),
    ...
    public static void aBucketThatWouldNotFitIsRefusedWhole(GameTestHelper helper) {
        ...
        reactor.inputTank()
                .fill(
                        new FluidStack(
                                Fluids.WATER,
                                StructureReactorBlockEntity.FLUID_TANK_CAPACITY_MB - room),
                        FluidAction.EXECUTE);
        IFluidHandlerItem water = container(new ItemStack(Items.WATER_BUCKET));
    ...
    public static void transfersAreLimitedByTheSmallerSide(GameTestHelper helper) {
        StructureReactorBlockEntity reactor = place(helper);
        int partial = 400;
        reactor.outputTank().fill(new FluidStack(Fluids.WATER, partial), FluidAction.EXECUTE);
        IFluidHandlerItem container = new PartialContainer(BUCKET);

        helper.assertTrue(
                reactor.exchangeWithFluidContainer(container), "the transfer must move fluid");
        helper.assertTrue(
                reactor.outputTank().isEmpty(), "the output tank must be drained completely");
        helper.assertTrue(
    ...
    public static void aPartlyFilledContainerIsToppedUpFromTheOutput(GameTestHelper helper) {
        StructureReactorBlockEntity reactor = place(helper);
        reactor.inputTank().fill(new FluidStack(Fluids.LAVA, BUCKET), FluidAction.EXECUTE);
        reactor.outputTank().fill(new FluidStack(Fluids.WATER, 500), FluidAction.EXECUTE);
