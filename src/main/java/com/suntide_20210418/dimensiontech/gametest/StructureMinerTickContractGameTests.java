...
public final class StructureMinerTickContractGameTests {
    private StructureMinerTickContractGameTests() {}
    ...
    public static void autoExtractFluidPrecedesPendingOutputRetry(GameTestHelper helper) {
        ...
        ServerLevel level = helper.getLevel();
        StructureMinerMultiblock.place(level, target.getBlockPos(), target.getMinerTier());

        BlockPos sourcePosition = target.getBlockPos().north();
        level.setBlock(sourcePosition, ModBlocks.TIER_2_STRUCTURE_MINER.get().defaultBlockState(), 3);
