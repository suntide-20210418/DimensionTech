...
public final class MythicCrucibleGameTests {
    ...
    public static void boundaryInputsConsumeAndResolve(GameTestHelper helper) {
        MythicCrucibleRecipe recipe = MythicCrucibleRecipes.all().stream().findFirst().orElseThrow();

        MythicCrucibleCycle immediate = new MythicCrucibleCycle();
        ...
        helper.assertTrue(result == MythicCrucibleCycle.Resolution.CORRECT,
                "tick 0 must advance without reward");

        MythicCrucibleCycle rewarded = new MythicCrucibleCycle();
        rewarded.start(recipe);
        for (int i = 0; i < MythicCrucibleCycle.REWARD_START_TICK; i++) rewarded.tick();
        helper.assertTrue(rewarded.resolve(new ItemStack(Items.QUARTZ))
                        == MythicCrucibleCycle.Resolution.CORRECT_REWARDED,
                "tick 20 must reward");

        MythicCrucibleCycle empty = new MythicCrucibleCycle();
        empty.start(recipe);
        empty.tick();
        helper.assertTrue(empty.resolve(ItemStack.EMPTY) == MythicCrucibleCycle.Resolution.NONE,
    ...
    public static void lateAndWrongInputsUsePenalties(GameTestHelper helper) {
        MythicCrucibleRecipe recipe = MythicCrucibleRecipes.all().stream().findFirst().orElseThrow();
        MythicCrucibleCycle late = new MythicCrucibleCycle();
        late.start(recipe);
        ...
        helper.assertTrue(phase == MythicCrucibleCycle.Resolution.PHASE_IDLE,
                "tick 101 must phase-idle");
        helper.assertTrue(phase.consumesInput(), "late input must be consumed");

        MythicCrucibleCycle wrong = new MythicCrucibleCycle();
        wrong.start(recipe);
        ...
        helper.assertTrue(penalty == MythicCrucibleCycle.Resolution.STABILIZE_FAILURE,
    ...
    public static void fifthTierAlternatesOnlyAfterCommit(GameTestHelper helper) {
        MythicCrucibleRecipe recipe = MythicCrucibleRecipes.all().stream()
                .filter(MythicCrucibleRecipe::hasAlternateBranch).findFirst().orElseThrow();
        MythicCrucibleCycle cycle = new MythicCrucibleCycle();
        cycle.start(recipe);
        ...
        helper.assertTrue(cycle.status() == MythicCrucibleCycle.Status.READY_TO_COMMIT,
                "branch A must reach commit state");
        helper.assertTrue(cycle.branch() == MythicCrucibleRecipe.Branch.A,
                "branch must not change before commit");
        cycle.commit();
        helper.assertTrue(cycle.branch() == MythicCrucibleRecipe.Branch.B,
                "branch must change once after commit");
        helper.succeed();
    }

    private static void resolve(MythicCrucibleCycle cycle, Item item) {
