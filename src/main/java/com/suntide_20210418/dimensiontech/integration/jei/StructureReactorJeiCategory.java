...
/**
 * The reactor's JEI page: input fluid, fragment, output fluid, and the ritual state sequence with
 ...
 * StructureReactorJeiRecipes#visibleRows(int)}) and anything longer is summarised instead of drawn.
 */
public final class StructureReactorJeiCategory
        extends AbstractRecipeCategory<StructureReactorJeiRecipe> {
     ...
     */
    public StructureReactorJeiCategory(IGuiHelper guiHelper) {
        this(
                guiHelper,
                StructureReactorJeiRecipes.visibleRows(
                        StructureReactorJeiRecipes.longestSequence(StructureReactorRecipes.all())));
    }

    private StructureReactorJeiCategory(IGuiHelper guiHelper, int rows) {
        super(
                StructureReactorJeiPlugin.RECIPE_TYPE,
                Component.translatable(StructureReactorJeiText.TITLE),
                guiHelper.createDrawableItemLike(ModBlocks.STRUCTURE_REACTOR.get()),
                WIDTH,
                heightFor(rows));
        this.rows = rows;
    }
    ...
    public void setRecipe(
            IRecipeLayoutBuilder builder, StructureReactorJeiRecipe recipe, IFocusGroup focuses) {
    ...
    private void addStepSlot(
            IRecipeLayoutBuilder builder, StructureReactorJeiRecipe recipe, int index) {
        StructureReactorJeiRecipe.DisplayStep step = recipe.steps().get(index);
        int number = index + 1;
        IRecipeSlotBuilder slot =
                builder.addSlot(RecipeIngredientRole.INPUT, STEP_X, ROW_START_Y + index * ROW_H)
                        ...
                        .addRichTooltipCallback(
                                (view, tooltip) ->
                                        StructureReactorJeiText.appendStepTooltip(
                                                tooltip, step, number));
    ...
    public void createRecipeExtras(
            IRecipeExtrasBuilder builder, StructureReactorJeiRecipe recipe, IFocusGroup focuses) {
        builder.addRecipeArrow().setPosition(ARROW_X, TOP_Y + 1);
        line(builder, StructureReactorJeiText.ioLine(recipe), TEXT_X, HEADER_Y0);
        line(builder, StructureReactorJeiText.branchLine(recipe), TEXT_X, HEADER_Y1);
        if (recipe.alternateBranches()) {
            line(builder, StructureReactorJeiText.alternateLine(), TEXT_X, HEADER_Y2);
        }
        line(
                builder,
                Component.translatable(StructureReactorJeiText.SEQUENCE),
                TEXT_X,
                SEQUENCE_TITLE_Y);
        ...
        for (int index = 0; index < drawable; index++) {
            line(
                    builder,
                    StructureReactorJeiText.rowLine(recipe.steps().get(index)),
                    STEP_TEXT_X,
        ...
        if (recipe.steps().size() > drawable) {
            line(
                    builder,
                    StructureReactorJeiText.moreLine(recipe.steps().size() - drawable),
                    STEP_TEXT_X,
                    ROW_START_Y + drawable * ROW_H + ROW_TEXT_INSET);
        }
        builder.addText(
                        StructureReactorJeiText.footerLines(recipe),
                        WIDTH - TEXT_X * 2,
    ...
    @Override
    public void draw(
            StructureReactorJeiRecipe recipe,
    ...
    public void getTooltip(
        ...
        StructureReactorJeiText.appendStepTooltip(tooltip, recipe.steps().get(index), index + 1);
    }
    ...
    private int drawableSteps(StructureReactorJeiRecipe recipe) {
    ...
    private static List<ItemStack> fragmentStacks(StructureReactorJeiRecipe recipe) {
