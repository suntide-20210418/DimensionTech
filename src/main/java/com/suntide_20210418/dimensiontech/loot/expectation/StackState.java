 ...
 * 来承载这个规范状态。1.21 里 {@code ItemStack} 本身就是 {@code (Item, DATA_COMPONENT_TYPE 补丁,
 * count)} 三元组，而组件值可能引用 datapack 注册表（附魔是 {@code Holder<Enchantment>}），
 * NBT 编解码因此必须带上 {@code HolderLookup.Provider}。既然身份判据本来就等价于
 * {@code Item + DataComponentPatch + count}（原版自己的 {@code isSameItemSameComponents} 也是这么比的），
 * 这里直接持有这三个字段：构造与 {@link #stack()} 不再依赖注册表，只有需要落到字符串的
 * {@link #serializedStackData(HolderLookup.Provider)} 才要求注册表上下文。
 */
public final class StackState {
    ...
    public String serializedStackData(HolderLookup.Provider registries) {
        ...
        // AIR cannot be encoded by ItemStack.CODEC (it uses ITEM_NON_AIR_CODEC); OPTIONAL_CODEC maps
