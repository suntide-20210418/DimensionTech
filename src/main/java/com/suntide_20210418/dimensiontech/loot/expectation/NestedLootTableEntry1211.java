 ...
 *
 * <p>1.21 replaced the 1.20.1 {@code LootTableReference} with {@code NestedLootTable} and renamed the
 * payload field from {@code name} to {@code value}:
 *
 * <pre>{@code
 * Codec.either(ResourceKey.codec(Registries.LOOT_TABLE), LootTable.DIRECT_CODEC).fieldOf("value")
 * }</pre>
 *
 * <p>{@code value} is an {@code Either}: a table id such as {@code minecraft:chests/simple_dungeon}, or a
 * whole inline table object. Vanilla 1.21.1 data (1178 tables in the default pack) uses the reference form
 * 31 times and the inline form 3 times ({@code equipment/trial_chamber}), and uses {@code name} zero
 * times — a 1.20.1-style {@code "name": ...} is rejected by the vanilla loader, so a table written that way
 * never reaches this engine. The entry type id itself did not change.
 *
 * <p>An inline table is not a registry entry and therefore has no id to recurse into; the engine reports it
 * as unsupported instead of quietly treating it as an empty table.
 */
public final class NestedLootTableEntry1211 {
    ...
    /**
     * The referenced table id, or {@code null} when {@code entry} is not a nested loot table entry, carries
     * an inline table, or is malformed. Self-guarding on the entry type so call sites cannot read the field
