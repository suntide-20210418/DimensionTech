 ...
 *
 * <p>The sixteen levels are partitioned by <em>what the reactor is waiting for</em>, because that is
 * the only thing a redstone circuit can act on. A level never means two things, so a single
 ...
 *   <li>{@code 5}-{@code 8} - the same four steps while the reward window is open, i.e. {@code level
 *       + }{@link #REWARD_WINDOW_OFFSET}. A circuit decodes the step with {@code (signal - 1) % 4}
 *       and the window with {@code signal >= 5}.
 *   <li>{@code 9}-{@code 12} - deliberately unassigned, so a future refinement stays inside the
 *       operation-phase half of the range.
 *   <li>{@link #IDLE_BLOCKED} - idle but blocked: the tank and the fragment slot disagree with every
 *       recipe, or the output tank has no room. Unlike {@link #IDLE} this one needs a player.
