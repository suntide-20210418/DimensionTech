...
public final class StructureValueSnapshot {
                    ...
                    .sorted(
                            Map.Entry.comparingByKey(
                                    Comparator.comparing(TerminalItem::itemId)
                                            ...
                                            .thenComparing(TerminalItem::rarity)))
                    .forEach(
                            entry -> {
        ...
        public String configFingerprint() {
            ...
            itemValues.forEach(
                    (id, values) -> {
                        ...
                        values.forEach(
                                (rarity, value) ->
                                        rarities.addProperty(rarity, Double.toHexString(value)));
    ...
    public record Result(
            ...
            Map<String, ExactProbability> itemCounts,
            String failure) {
        public Result {
    ...
    public static Result calculate(Expectation input, Config config) {
        ...
        try {
            ...
            for (var entry : input.occurrences().entrySet()) {
                ...
                if (multiplier == null)
                    throw new IllegalArgumentException("Missing frozen rarity " + item.rarity());
                ExactProbability count =
                        entry.getValue().multiply(ExactProbability.of(item.count(), 1));
            ...
            return new Result(
                    false,
                    Double.isFinite(dimension) && dimension >= 0 ? dimension : 0,
                    0,
                    new Expectation(Map.of()),
                    Map.of(),
                    error.getMessage());
        }
    }
