...
class RandomStateDistributionTest {
    ...
    void equalValuesWithDifferentContinuationStatesRemainDistinct() {
        ...
        RandomStateDistribution<String> distribution = RandomStateDistribution.of(Map.of(
                new RandomStateDistribution.State<>("same", first), ExactProbability.of(1, 2),
                new RandomStateDistribution.State<>("same", second), ExactProbability.of(1, 2)));
    ...
    void deterministicRandomTransitionCarriesTheExactContinuation() {
        ...
        RandomStateDistribution<Integer> result = RandomStateDistribution
                .singleton(0, initial)
                .flatMap(state -> {
                    var draw = state.randomState().nextInt(7);
                    return RandomStateDistribution.singleton(draw.value(), draw.state());
                }, 10);

        var expected = initial.nextInt(7);
        assertEquals(
                ExactProbability.ONE,
                result.masses().get(new RandomStateDistribution.State<>(
