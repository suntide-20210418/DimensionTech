...
class RandomTraceDistributionTest {
    ...
    void flatMapRetainsDifferentOrderedCallPathsAndMarginalizesOnlyOnRequest() {
        ...
        RandomTraceDistribution<String> result = RandomTraceDistribution.of(first).flatMap(
                value -> value
                        ? RandomTraceDistribution.of(Map.of(
                                new Outcome<>("done", List.of(
        ...
        assertEquals(
                List.of(1, 2),
                result.masses().keySet().stream().map(outcome -> outcome.calls().size()).sorted().toList());
    }
}
